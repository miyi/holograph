...
const loadGQLDocs = () => {
  const typeDefDocs = loadFiles(
    join(process.cwd(), 'src', 'modules', '**/*.graphql'),
  )
  const resolverDocs = loadFiles(
    join(process.cwd(), 'src', 'modules', '**/resolvers.ts'),
  )
  return { typeDefDocs, resolverDocs }
}

const mergeGQLDocs = (typeDefDocs: Array<any>, resolverDocs: Array<any>) => {
